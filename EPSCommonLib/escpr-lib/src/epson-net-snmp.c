...
EPS_ERR_CODE snmpGetInkInfo_Page(
	...
	for(i = 0; EPS_ERR_NONE == ret && i < pstInfo->nInkNo; i++){
		if(ASN_VT_INTEGER == pdu.type){
			/*EPS_DBGPRINT(("%d\n", pdu.val.v_long));*/
			pstInfo->nColor[i] = serInkLevelNromalize(pdu.val.v_long * 100 / nMax[i]);
		}
...
EPS_ERR_CODE snmpInfoCommand_Page(
	...
	EPS_LOG_FUNCIN
	if( !(EPS_CBTCOM_PM == cmd || EPS_CBTCOM_PM2 == cmd)  ){
...
static EPS_ERR_CODE SnmpFindRecv(
	...
	*printer = (EPS_PRINTER_INN*)EPS_ALLOC( sizeof(EPS_PRINTER_INN) );
	...
	devIdRes = serParseDeviceID(infoBuf, strlen(infoBuf),
	...
	if( EPS_LANG_ESCPR == (*printer)->language ){
		...
		if(EPS_ERR_NONE != ret){
			...
			ret = snmpInfoCommand( *printer, EPS_CBTCOM_CD, (EPS_UINT8*)infoBuf, &infoBufSize);
			if(EPS_ERR_NONE == ret){
...
static EPS_ERR_CODE  CreateCommand(
	...
	/* Community */
	pPos = snmpMakeStrField(community, strlen(community), pPos);
...
EPS_LOG_FUNCIN
	...
	/* check Error */
	if( SNMP_ERR_NONE != response.errorStatus ){
		EPS_SNMP_REREASE_PDU(response)
		EPS_RETURN( EPS_ERR_COMM_ERROR )
	}
