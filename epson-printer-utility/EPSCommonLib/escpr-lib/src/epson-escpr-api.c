...
/*******************************************|********************************************/
    ...
    /*** -------------------------------------------------------------------------------*/
static EPS_INT32    libStatus;                  /*  Library (epsInitDriver) status      */
...
static EPS_INT32    gStatusCount;   /* Variable for register the number of getting      */
    ...
    /*** -------------------------------------------------------------------------------*/
static EPS_UINT32   sendDataBufSize = 0;
static EPS_UINT8*   sendDataBuf;    /* buffer of SendCommand(save) input                */
static EPS_UINT32   tmpLineBufSize;
static EPS_UINT8*   tmpLineBuf;
...
static EPS_ERR_CODE SetupJobAttrib          (const EPS_JOB_ATTRIB*                      );
static EPS_ERR_CODE SetupRGBAttrib          (void                                       );
static EPS_ERR_CODE SetupJPGAttrib          (void                                       );
static EPS_ERR_CODE SendStartJob			(EPS_BOOL                                   );
static EPS_ERR_CODE SendEndJob  			(EPS_BOOL                                   );

static EPS_ERR_CODE PrintBand               (const EPS_UINT8*, EPS_UINT32, EPS_UINT32*  );
...
static EPS_ERR_CODE CalcPrintableArea		(EPS_JOB_ATTRIB *pAttr, EPS_PRINT_JOB *pJobInfo,
											 EPS_UINT32* printableWidth, EPS_UINT32* printableHeight);
...
/*** Copy paper count data to input buffer ***/
	status->paper_count.color = stInfo.paper_count.color;
	status->paper_count.monochrome = stInfo.paper_count.monochrome;
	status->paper_count.blank = stInfo.paper_count.blank;
	status->paper_count.adf = stInfo.paper_count.adf;
	status->paper_count.color_borderless = stInfo.paper_count.color_borderless;
	status->paper_count.monochrome_borderless = stInfo.paper_count.monochrome_borderless;
...
/*** Validate/Confirm Page Attribute Data                                               */
    ...
    if (! ( ( (jobAttr->mediaSizeIdx       >= EPS_MSID_A4              ) &&
              (jobAttr->mediaSizeIdx       <= EPS_MSID_KAKU_20         )    ) ||
...
/*              (jobAttr->mediaTypeIdx       <= EPS_MTID_GLOSSYHAGAKI    )    ) ||*/
              (jobAttr->mediaTypeIdx       <= EPS_MTID_COLOR           )    ) ||
...
/*** Get Printable Area                                                                 */
	retStatus = CalcPrintableArea(jobAttr, NULL, printableWidth, printableHeight);
...
/*======================================================================================*/
	...
	case EPS_CP_JPEG:				/* Jpeg */
		...
		retStatus = CalcPrintableArea(&printJob.attr, &printJob, NULL, NULL);
		if( EPS_ERR_NONE != retStatus ){
			EPS_RETURN( retStatus );
		}
		break;
...
EPS_ERR_CODE    CalcPrintableArea (
	...
	EPS_INT32       paperWidth, paperHeight;
	EPS_INT32       tempPrintableWidth, tempPrintableHeight;
	const EPS_MEDIA_INFO* pMI = NULL;
	EPS_INT16		borderPixelsW, borderPixelsH, borderlessRight, borderlessBottom;

	EPS_LOG_FUNCIN;

/*** Initialize Global/Local Variables                                                  */
    idx                 = -1;
    tempPrintableWidth  = tempPrintableHeight = 0;
	paperWidth = paperHeight = 0;
	borderPixelsW = borderPixelsH = borderlessRight = borderlessBottom = 0;

	/*** Input Image Resolution                                                     */
    /*** Select table and factor                                                    */
	if(pAttr->inputResolution == EPS_IR_360X360){
		pMI = epsMediaSize;
	    factor = 1;
		borderPixelsW = borderPixelsH = EPS_BORDERS_MARGIN_360;
		borderlessRight = EPS_RIGHT_MARGIN__BORDERLESS_360;
		borderlessBottom = EPS_BOTTOM_MARGIN__BORDERLESS_360;
		if(EPS_MSID_ENV_C4_P == pAttr->mediaSizeIdx || EPS_MSID_KAKU_20 == pAttr->mediaSizeIdx){
			borderPixelsW = 141;
		} else if((EPS_MSID_ENV_10_L <= pAttr->mediaSizeIdx && EPS_MSID_YOKEI_4 >= pAttr->mediaSizeIdx) ||
			      (EPS_MSID_ENV_10_P <= pAttr->mediaSizeIdx && EPS_MSID_NEWENV_P >= pAttr->mediaSizeIdx) ){
			borderPixelsW = 70;
		}
	} else if(pAttr->inputResolution == EPS_IR_720X720){
		pMI = epsMediaSize;
		factor = 2;
		borderPixelsW = borderPixelsH = EPS_BORDERS_MARGIN_360;
		borderlessRight = EPS_RIGHT_MARGIN__BORDERLESS_360;
	...
	} else if(pAttr->inputResolution == EPS_IR_300X300){
		pMI = epsMediaSize300;
	    factor = 1;
		borderPixelsW = borderPixelsH = EPS_BORDERS_MARGIN_300;
		borderlessRight = EPS_RIGHT_MARGIN__BORDERLESS_300;
		borderlessBottom = EPS_BOTTOM_MARGIN__BORDERLESS_300;
		if(EPS_MSID_ENV_C4_P == pAttr->mediaSizeIdx || EPS_MSID_KAKU_20 == pAttr->mediaSizeIdx){
			borderPixelsW = 118;
		} else if((EPS_MSID_ENV_10_L <= pAttr->mediaSizeIdx && EPS_MSID_YOKEI_4 >= pAttr->mediaSizeIdx) ||
			      (EPS_MSID_ENV_10_P <= pAttr->mediaSizeIdx && EPS_MSID_NEWENV_P >= pAttr->mediaSizeIdx) ){
	...
	} else if(pAttr->inputResolution == EPS_IR_600X600){
		pMI = epsMediaSize300;
	    factor = 2;
		borderPixelsW = borderPixelsH = EPS_BORDERS_MARGIN_300;
		borderlessRight = EPS_RIGHT_MARGIN__BORDERLESS_300;
    ...
    /*** Margin                                                                     */
	if (pAttr->printLayout == EPS_MLID_CUSTOM) {
		if (pAttr->topMargin    < borderPixelsH*factor) EPS_RETURN( EPS_ERR_INV_TOP_MARGIN );
		if (pAttr->leftMargin   < borderPixelsW*factor) EPS_RETURN( EPS_ERR_INV_LEFT_MARGIN );
		if (pAttr->bottomMargin < borderPixelsH*factor) EPS_RETURN( EPS_ERR_INV_BOTTOM_MARGIN );
		if (pAttr->rightMargin  < borderPixelsW*factor) EPS_RETURN( EPS_ERR_INV_RIGHT_MARGIN );
	}
...
/*======================================================================================*/
	...
	for (idx = 0; pMI[idx].id != -1; idx++) {
		if (pMI[idx].id == pAttr->mediaSizeIdx)
			break;
	}
	if (pMI[idx].id == -1) {
		EPS_RETURN( EPS_ERR_INV_MEDIA_SIZE );
	...
	/*** paper size                                                                     */
	if( pAttr->mediaSizeIdx != EPS_MSID_USER ){
		paperWidth  = pMI[idx].paper_x * factor;
		paperHeight = pMI[idx].paper_y * factor;
	} else{
		if(pAttr->userDefWidth == 0 || pAttr->userDefHeight == 0){
			EPS_RETURN( EPS_ERR_INV_MEDIA_SIZE );
		}
	...
	case EPS_MLID_BORDERLESS:
		if( pAttr->mediaSizeIdx != EPS_MSID_USER ){
			...
			tempPrintableWidth      = paperWidth - 
										((pMI[idx].left_margin_borderless + borderlessRight) * factor);
			tempPrintableHeight     = paperHeight - 
										((pMI[idx].top_margin_borderless + borderlessBottom) * factor);
		}

		if(pJobInfo != NULL){
			pJobInfo->topMargin      = pMI[idx].top_margin_borderless   * factor;
			pJobInfo->leftMargin     = pMI[idx].left_margin_borderless  * factor;
			pJobInfo->borderlessMode = EPS_BORDERLESS_NORMAL;
		}
		break;
	...
	case EPS_MLID_DIVIDE16:  
		if( pAttr->mediaSizeIdx != EPS_MSID_USER ){
			...
			tempPrintableWidth      = paperWidth - (borderPixelsW * factor * 2);
			tempPrintableHeight     = paperHeight - (borderPixelsH * factor * 2);
		}

		if(pJobInfo != NULL){
			pJobInfo->topMargin      = borderPixelsH * factor;
			pJobInfo->leftMargin     = borderPixelsW * factor;
			pJobInfo->borderlessMode = EPS_BORDER_3MM_MARGINE;
		}
		break;

	case EPS_MLID_CDLABEL:
		tempPrintableWidth  =
		tempPrintableHeight = elGetDots(pAttr->inputResolution, pAttr->cdDimOut*10);
		if(pJobInfo != NULL){
			pJobInfo->topMargin  = CDDVD_OFFSET_Y(pAttr->inputResolution, pAttr->cdDimOut);
			pJobInfo->leftMargin = CDDVD_OFFSET_X(pAttr->inputResolution, pAttr->cdDimOut);
			pJobInfo->borderlessMode = EPS_BORDER_3MM_MARGINE; /* It's no meaning & no effect */

			/*EPS_DBGPRINT(("dim: %d / top: %d / left: %d\n", pAttr->cdDimOut,
							pJobInfo->topMargin, pJobInfo->leftMargin));*/
		}
		break;

	default: /* pAttr->printLayout  == EPS_MLID_CUSTOM */
		tempPrintableWidth      = paperWidth  - pAttr->leftMargin - pAttr->rightMargin;
		tempPrintableHeight     = paperHeight - pAttr->topMargin  - pAttr->bottomMargin;
		if(pJobInfo != NULL){
			pJobInfo->topMargin      = pAttr->topMargin;
			pJobInfo->leftMargin     = pAttr->leftMargin;
	...
	if (pAttr->printLayout == EPS_MLID_CUSTOM || 
		pAttr->mediaSizeIdx == EPS_MSID_USER  ){
    ...
    if ((tempPrintableWidth > 0) && (tempPrintableHeight > 0)) {
		...
		if(pJobInfo != NULL){
			...
			/*** BORDER                                                                 */
			pJobInfo->border.top        =
			pJobInfo->border.bottom     = borderPixelsH * factor;
			pJobInfo->border.left       =
			pJobInfo->border.right      = borderPixelsW * factor;
		}
...
/*                                                                                      */
/*******************************************|********************************************/
static EPS_ERR_CODE    SendStartJob (
...
/*                                                                                      */
/*******************************************|********************************************/
static EPS_ERR_CODE    SendEndJob (
...
/*		EPS_DBGPRINT(("Size=%d\r\n", pmString[pmIdx+1]));*/
		...
		for(tIdx = 0; tIdx < num_mType; tIdx++) {
			...
			if( EPS_MTID_3D == pmString[pmIdx+1] ){
				innerPrinter->supportedMedia.sizeList[sIdx].numTypes--;
					...
					for(tIdx = 0; tIdx < pMSize->numTypes; tIdx++){
						if(pmString[pmIdx] == pMSize->typeList[tIdx].mediaTypeID){
				            ...
				            if( 0 == pMSize->typeList[tIdx].paperSource ){
								pMSize->typeList[tIdx].paperSource = serComplementPaperSource(pMSize->mediaSizeID);
							}
