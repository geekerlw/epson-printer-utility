...
EPS_ERR_CODE usbFind (
    ...
    debug_msg("start call function epson-usb.c/usbFind \n");
...
/***------------------------------------------------------------------------------------*/
    ...
    debug_msg("Exit call function epson-usb.c/usbFind \n");
	EPS_RETURN( ret );
}
...
/*** Get PM from Printer                                                                */
 	if(1 == type){
		ret = InfoCommand(fd, ((EPS_USB_PRINTER_INFO*)printer->protocolInfo)->usbProtocol,
						EPS_CBTCOM_PM, pString, bufSize);
	} else if(2 == type){
		ret = InfoCommand(fd, ((EPS_USB_PRINTER_INFO*)printer->protocolInfo)->usbProtocol,
						EPS_CBTCOM_PM2, pString, bufSize);
	} else{
...
EPS_ERR_CODE    usbGetStatus (
    ...
    debug_msg("start call function epson-usb.c/usbGetStatus \n");
/*** Declare Variable Local to Routine                                                  */
...
/*** Return to Caller                                                                   */
    debug_msg("Exit call function epson-usb.c/usbGetStatus \n");
    EPS_RETURN( ret );
}
...
EPS_LOG_FUNCIN;
	...
	ret = InfoCommand(fd, usbProtocol, EPS_CBTCOM_ST, Status, &Size);
	if( EPS_ERR_NONE != ret ){
...
static EPS_ERR_CODE     ProbeESCPR (
    ...
    debug_msg("start call function epson-usb.c/ProbeESCPR \n");
/*** Declare Variable Local to Routine                                                  */
...
/*** Initialize Local Variables                                                         */
    ...
    debug_msg("Exit call function epson-usb.c/ProbeESCPR \n");
    EPS_RETURN( retStatus );
}
...
static EPS_ERR_CODE     CreatePrinterInfo (
    ...
    debug_msg("start call fucntion epson-usb.c/CreatePrinterInfo \n");
/*** Declare Variable Local to Routine                                                  */
...
CreatePrinterInfo_ERROR:
    ...
    debug_msg("Exit call fucntion epson-usb.c/CreatePrinterInfo \n");
    EPS_RETURN( retStatus );
}
...
static EPS_ERR_CODE   PortResolution(
    ...
    debug_msg("start call fucntion epson-usb.c/PortResolution \n");
...
EPS_LOG_FUNCIN;
	...
	*pFd = EPS_INVALID_FILEDSC;
    ...
    debug_msg("Exit call fucntion epson-usb.c/PortResolution \n");
...
static EPS_ERR_CODE   ProbePrinterByName (
    ...
    debug_msg("start call fucntion epson-usb.c/ProbePrinterByName \n");
/*** Declare Variable Local to Routine                                                  */
    EPS_ERR_CODE	ret = EPS_ERR_NONE;			 /* Return status of internal calls     */
...
EPS_LOG_FUNCIN;
    ...
    debug_msg("Exit call fucntion epson-usb.c/ProbePrinterByName \n");
...
static EPS_ERR_CODE   InfoCommand (
    ...
    debug_msg("start call fucntion epson-usb.c/InfoCommand \n");
    EPS_ERR_CODE    ret = EPS_ERR_NONE;
    ...
    debug_msg("Exit call fucntion epson-usb.c/InfoCommand \n");
    EPS_RETURN( ret );
}
...
static EPS_ERR_CODE   CbtInfoCommand (
    ...
    debug_msg("start call fucntion epson-usb.c/CbtInfoCommand \n");
    ...
    debug_msg("Exit call fucntion epson-usb.c/CbtInfoCommand \n");
    EPS_RETURN( EPS_ERR_NONE );
}
...
//    debug_msg("start call fucntion epson-usb.c/RmtInfoCommand \n");
    ...
    EPS_UINT8 rspBuff[EPS_PM_MAXSIZE];
	...
	if(sizeof(rspBuff) < packetNum * packetSize){
	...
	*Size = totalRespSize;
    ...
    debug_msg("Exit call fucntion epson-usb.c/RmtInfoCommand \n");
    EPS_RETURN( EPS_ERR_NONE );
}
...
//    debug_msg("start call function epson-usb.c/OpenPortal\n");
	EPS_INT32 ret = 0;
...
//    debug_msg("Exit call function epson-usb.c/OpenPortal err: %d\n", ret);
	if(0 == ret){
		EPS_RETURN( EPS_ERR_NONE );
	} else{
		EPS_RETURN( EPS_ERR_NOT_OPEN_IO );
...
static void   ParseUsbProtocol (
    ...
    debug_msg("start call functon epson-usb.c/ParseUsbProtocol \n");
    EPS_INT8*       i = NULL;
    EPS_INT8*       j = NULL;
    EPS_INT8*       k = NULL;
...
*/
	...
	*j = ';';
    ...
    debug_msg("Exit call functon epson-usb.c/ParseUsbProtocol \n");
    EPS_RETURN_VOID;
}
...
static EPS_ERR_CODE   GetSerialNumber (
    ...
    debug_msg("start call function epson-usb.c/GetSerialNumber\n");
	EPS_ERR_CODE ret = EPS_ERR_NONE;
	EPS_FILEDSC fd;
	EPS_INT8*   tmpBuff = NULL;
	...
	if( EPS_USBP_CBT == usbInfo->usbProtocol ){
		ret = cbtCommChannelOpen(fd, EPS_CBTCHANNEL_CTRL, TRUE);
		if (EPS_ERR_NONE == ret ) {
			ret = InfoCommand(fd, usbInfo->usbProtocol, EPS_CBTCOM_CD, (EPS_UINT8*)tmpBuff, &tmpBuffSize);
			if (EPS_ERR_NONE == ret ) {
				/*retStatus = */serGetSerialNo(tmpBuff, tmpBuffSize, serialNo);
			} else{
		...
		ret = RmtInfoCommand(fd, EPS_CBTCOM_ST, (EPS_UINT8*)tmpBuff, &tmpBuffSize);
		if (EPS_ERR_NONE == ret ) {
			/*retStatus = */serGetSerialNoFormST(tmpBuff, serialNo, EPS_ADDR_BUFFSIZE);
		} else{
			ret = EPS_ERR_NONE; /* ignoure */
		}
	}
    ...
    debug_msg("Exit call function epson-usb.c/GetSerialNumber\n");
	EPS_RETURN( ret );
}
