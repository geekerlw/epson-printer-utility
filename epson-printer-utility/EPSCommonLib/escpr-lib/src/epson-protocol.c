...
EPS_ERR_CODE prtFindPrinter (
    ...
    debug_msg("start call function epson-protocol.c/prtFindPrinter \n");
...
/***------------------------------------------------------------------------------------*/
    ...
    debug_msg("Exit call function epson-protocol.c/prtFindPrinter \n");
...
//    debug_msg("start call function epson-protocol.c/prtGetInkInfo \n");
/*** Declare Variable Local to Routine                                                  */
	EPS_ERR_CODE    ret = EPS_ERR_NONE;         /* Return status of internal calls      */
...
/*** protocol GetStatus                                                                 */
	...
	if(EPS_ERR_NONE == ret
		...
		for(idx=0; idx < EPS_INK_NUM; idx++) {
			...
			info->status[idx]    = lclStatus.nColorStatus[idx];
			info->showInkInfo	 = lclStatus.showInkInfo;
			info->showInkLow	 = lclStatus.showInkLow;
		}
	}
//    debug_msg("Exit call function epson-protocol.c/prtGetInkInfo \n");
...
//    debug_msg("start call function epson-protocol.c/prtRegPrinter \n");
...
//    debug_msg("Exit call function epson-protocol.c/prtRegPrinter \n");
	EPS_RETURN( EPS_ERR_NONE )
}
...
EPS_BOOL prtIsRegistered(
    ...
    debug_msg("start call function epson-protocol.c/prtIsRegistered \n");
	...
	while(NULL != current) {
		...
		if(TRUE == bMatch){
			...
			if (EPS_PRT_PROTOCOL(current->printer->protocol) <= EPS_PRT_PROTOCOL(Protocol)) {
                debug_msg("Exit call function epson-protocol.c/prtIsRegistered \n");
				EPS_RETURN( TRUE )
			} else{
                debug_msg("Exit call function epson-protocol.c/prtIsRegistered \n");
...
void prtClearPrinterList(
    ...
    debug_msg("start call function prtClearPrinterList \n");
	EPS_PL_NODE* pCur = epsPrinterList.root;
    ...
    debug_msg("Exit call function prtClearPrinterList \n");
...
void prtSetIdStr(
    ...
    debug_msg("start call fucntion epson-protocol.c/prtSetIdStr \n");
	sprintf(printer->printerID, EPS_PRINTER_ID_STR, 
        EPS_PRT_PROTOCOL_EX(printer->protocol), idString);

    debug_msg("Exit call fucntion epson-protocol.c/prtSetIdStr \n");
}
